-- Fix the send_broadcast_to_class function to use class_section instead of class_id
-- Students table has class_section (e.g., 'CS-301-A - MORNING'), not class_id
-- This migration fixes the broadcast messaging to actually deliver messages to students
...
DECLARE
    ...
    v_class_session VARCHAR(50);
    v_class_section VARCHAR(255);
    v_student RECORD;
    v_conversation_id TEXT;
    v_message_id TEXT;
    v_total_recipients INTEGER := 0;
BEGIN
    -- Get class name and session with explicit type casting
    SELECT name, session INTO v_class_name, v_class_session
    FROM classes 
    WHERE id::text = p_class_id;
    
    IF v_class_name IS NULL THEN
        RAISE EXCEPTION 'Class not found with id: %', p_class_id;
    END IF;
    
    -- Build the class_section string to match students table format
    -- Format: "CS-301-A - MORNING"
    ...
    -- Send to each student in the class by matching class_section
    FOR v_student IN 
        SELECT 
            s.id::text as id, 
            CONCAT(s.first_name, ' ', s.last_name) as name
        FROM students s
        WHERE s.class_section = v_class_section
    LOOP
...
COMMENT ON FUNCTION send_broadcast_to_class IS 'Sends a message to all students in a class using class_section matching';
