...
export function ReportsDashboard({ className }: ReportsDashboardProps) {
  ...
  const [selectedReportData, setSelectedReportData] = React.useState<any>(null)
  ...
  // Mock report data - will be replaced with actual API calls
  const [reports, setReports] = React.useState<ReportType[]>([
    ...
    },
    {
      ...
      color: 'purple',
      lastGenerated: new Date(Date.now() - 8 * 60 * 60 * 1000), // 8 hours ago
      dataCount: 156,
      isLoading: false
    }
  ])
  ...
  const handleGenerateReport = async (reportId: string) => {
    ...
    try {
      // Convert basic filters to advanced filters format for API call
      const apiFilters = {
        dateRange: advancedFilters.dateRange,
        classIds: advancedFilters.classes.selectedIds,
